...
The databricks CLI bundles top level is defined by file `ml_0730/databricks.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`ml_0730/resources/model-workflow-resource.yml`)
 - batch inference workflow (`ml_0730/resources/batch-inference-workflow-resource.yml`)
 - monitoring resource and workflow (`ml_0730/resources/monitoring-resource.yml`)
 - feature engineering workflow (`ml_0730/resources/feature-engineering-workflow-resource.yml`)
 - model definition and experiment definition (`ml_0730/resources/ml-artifacts-resource.yml`)
...
If you want to use different catalog names, please update the `targets` declared in the `ml_0730/databricks.yml` and `ml_0730/resources/ml-artifacts-resource.yml` files.
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` deployment target is used by ML engineers to deploy ML resources to development workspace with `dev` configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-ml-0730-model     | /dev-ml-0730-experiment     |
| staging     | The `staging` deployment target is part of the CD pipeline. Latest main content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-ml-0730-model | /staging-ml-0730-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest release content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-ml-0730-model    | /prod-ml-0730-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the main branch, upon making a PR, an integration test will be triggered and ML resources deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-ml-0730-model    | /test-ml-0730-experiment    |
...
2. Complete the `TODO` in `ml_0730/databricks.yml` to add the dev workspace URI under `targets.dev.workspace.host`.
...
```
Following this, update the batch_inference_job base parameters in `ml_0730/resources/batch-inference-workflow-resource.yml` to pass
...
To get started, open `ml_0730/resources/batch-inference-workflow-resource.yml`.  The file contains the ML resource definition of a batch inference job, like:
...
The example above defines a Databricks job with name `${bundle.target}-ml-0730-batch-inference-job`
that runs the notebook under `ml_0730/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `resources/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.target}-ml-0730-batch-inference-job`.
...
`test-ml-0730-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-ml-0730-batch-inference-job`.
